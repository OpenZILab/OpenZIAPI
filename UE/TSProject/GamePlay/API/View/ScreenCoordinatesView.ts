...
export class ScreenCoordinatesView extends BaseView {
    ...
    RefreshCoordinates(){
        ...
        CurScreenPosition.Y - this.lastScreenLoction.Y < -0.01)){
            ...
            let message = PackBroadcastMessage(msg, {id:this.id,ScreenPosition: {X:this.lastScreenLoction.X, Y:this.lastScreenLoction.Y}})
    ...
    ClearAllData(): void{
        if (this.tickID !== 0){
            Timer.GetInstance().DelTimer(this.tickID)
            this.tickID = 0
        }
        this.SceneCoordinates = new UE.Vector(0,0,0)
        this.lastScreenLoction = new UE.Vector2D(0,0)
